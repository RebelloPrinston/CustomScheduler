...
func NewPodCertificateRequestController(
	...
	_, _ = reqInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func (cc *PodCertificateRequestController) processNextWorkItem(ctx context.Context) bool {
	...
	if err := cc.syncFunc(ctx, cKey); err != nil {
		...
		var ign ignorableError
		if ignorable := errors.As(err, &ign); !ignorable {
			utilruntime.HandleError(fmt.Errorf("sync %v failed: %w", cKey, err))
		} else {
