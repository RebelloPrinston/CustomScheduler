...
func (s *podClientSigner) handle(ctx context.Context, req *certificatesv1alpha1.PodCertificateRequest) error {
	...
	for _, c := range req.Status.Conditions {
		if c.Type == certificatesv1alpha1.PodCertificateRequestConditionTypeDenied && c.Status == metav1.ConditionTrue {
			return nil
		}
		if c.Type == certificatesv1alpha1.PodCertificateRequestConditionTypeFailed && c.Status == metav1.ConditionTrue {
			return nil
		}
	}
	...
	if err := kubernetesx509.AddPodIdentityToCertificate(pi, tmpl); err != nil {
		return fmt.Errorf("while adding pod identity extension: %w", err)
	}
	...
	req.Status.CertificateChain = string(pemBytes)
	req.Status.IssuedAt = timePointer(metav1.NewTime(s.clock.Now()))
	req.Status.NotBefore = timePointer(metav1.NewTime(notBefore))
	req.Status.BeginRefreshAt = timePointer(metav1.NewTime(beginRefreshAt))
	req.Status.NotAfter = timePointer(metav1.NewTime(notAfter))
...
func timePointer(in metav1.Time) *metav1.Time {
	val := in
	return &val
}
