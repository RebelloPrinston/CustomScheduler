...
func TestConfigSprint(t *testing.T) {
	...
	for _, f := range []string{"%s", "%v", "%+v", "%#v"} {
		...
		if diff := cmp.Diff(got, want); diff != "" {
			t.Errorf("bad fmt.Sprintf output; diff (-got +want)\n%s", diff)
		}
	}
}
