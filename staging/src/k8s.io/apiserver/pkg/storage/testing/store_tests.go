...
func (f *FakeAggregatorFactory) Aggregate(key string, err error) bool {
...
func RunTestGetListWithErrorAggregation(t *testing.T, newStoreFn func(*testing.T) (context.Context, *FakeAggregatorFactory, InterfaceWithCorruptTransformer)) {
	tests := []struct {
		name           string
		featureEnabled bool
		n              int
		verifier       func(*testing.T, *FakeAggregatorFactory, error)
	}{
		{
			...
			n:              6, // object 2, 4, and 6 will go corrupt
			verifier: func(t *testing.T, aggr *FakeAggregatorFactory, errGetList error) {
				// the error returned from GetList should not be wrapped
				if want := "unable to transform key"; !strings.Contains(errGetList.Error(), want) {
					t.Errorf("expected the error to start with %q, but got: %v", want, errGetList)
				}
				if want, got := 1, len(aggr.got); want != got {
					t.Fatalf("expected exactly %d error(s), but got: %d", want, got)
				}
				// nolint:errorlint // this is a controlled test, so assert with
				// identity, the aggregator should return the error as is
				if want, got := errGetList, aggr.got[0]; want != got {
		...
		},
		{
			...
			verifier: func(t *testing.T, aggr *FakeAggregatorFactory, errGetList error) {
				...
				if want, got := 3, len(aggr.got); want != got {
					t.Fatalf("expected exactly %d error(s), but got: %d", want, got)
				}
			},
		},
		{
			name:           "feature enabled, should not exceed aggregattion limit",
			featureEnabled: true,
			...
			verifier: func(t *testing.T, aggr *FakeAggregatorFactory, errGetList error) {
				...
				limit := 100
				if want, got := limit+1, len(aggr.got); want != got {
					t.Fatalf("expected exactly %d error(s), but got: %d", want, got)
				}
