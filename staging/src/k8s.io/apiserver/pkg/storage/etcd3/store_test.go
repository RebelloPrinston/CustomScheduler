...
func TestGetListWithErrorAggregation(t *testing.T) {
	storagetesting.RunTestGetListWithErrorAggregation(t, func(t *testing.T) (context.Context, *storagetesting.FakeAggregatorFactory, storagetesting.InterfaceWithCorruptTransformer) {
		ctx, s, _ := testSetup(t)
		...
		wrapper := &storagetesting.FakeAggregatorFactory{}
...
func (f *corruptedTransformer) TransformFromStorage(ctx context.Context, data []byte, dataCtx value.Context) (out []byte, stale bool, err error) {
	if f.shouldBeCorrupt != nil && !f.shouldBeCorrupt(ctx, data) {
		return f.Transformer.TransformFromStorage(ctx, data, dataCtx)
	}
...
type storeWithCorruptedTransformer struct {
	storage.Interface
	// we need the original *store to mutate the transformer
	store *store
}
